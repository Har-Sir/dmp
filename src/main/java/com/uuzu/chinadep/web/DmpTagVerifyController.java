...
@Slf4j
@Controller
@Api(description = "dmp-chinadep OpenApi",tags = "dmp用户画像标签验证")
public class DmpTagVerifyController {
    ...
    @PostMapping("/tagverify")
    @ApiOperation(value = "dmp用户画像标签验证" , notes = "dmp用户画像标签验证")
    ...
    public  ResponseEntity<DmpResponse> post(@RequestBody @Validated DmpRequest dmpRequest) throws Exception {
        ...
        try {
            ...
            //获取数据
            retState = personaService.invokeEasy(dmpRequest.getIdType(), dmpRequest.getExid(), dmpRequest.getDataRange(), dmpResponse);
            if (!retState.getFlag()) {
